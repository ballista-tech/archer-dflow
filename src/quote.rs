...
mod tests {
    ...
    /// Degenerate lot/tick geometry: 35 base decimals and a tick near
    /// u64::MAX make a fill pass cost zero quote lots while draining base a
    /// sliver at a time, so the budget never shrinks. On chain this exhausts
    /// the compute budget; here the structural pass bound must refuse it, fast.
    #[test]
