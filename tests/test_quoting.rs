#[cfg(test)]
mod simulations {
    //! Quoting tests for the Archer AMM integration with DFlow.
    //!
    //! The tests ensure:
    //! - The AMM loads on-chain state correctly via the Amm trait lifecycle
...
use litesvm::LiteSVM;
    ...
    use archer_dflow::ARCHER_PROGRAM_ID;
    ...
    fn snapshot_state_into_svm(
        ...
        // Detect token programs from cached state
        let header = amm.market_header.as_ref().unwrap();
    ...
    fn sim_quote_request(
        ...
        let mut account_a =
            Account::new(LAMPORTS_PER_SOL, TokenAccount::LEN, &input_token_program);
        ...
        let mut account_b =
            Account::new(LAMPORTS_PER_SOL, TokenAccount::LEN, &output_token_program);
        ...
        litesvm.set_account(source_ata, account_a.into()).unwrap();
        litesvm.set_account(dest_ata, account_b.into()).unwrap();
    ...
    async fn load_amm(rpc_url: &str) -> (ArcherAmm, RpcClient) {
        let market_key_str =
            env::var("ARCHER_MARKET_KEY").expect("ARCHER_MARKET_KEY must be set");
